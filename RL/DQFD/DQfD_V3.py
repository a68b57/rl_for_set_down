...
class DQfD:
	...
	def build_layers(self, state, c_names, units_1, w_i, b_i, reg=None):
		a_d = self.action_dim
		with tf.variable_scope('l1'):
			...
			dense1 = tf.nn.sigmoid(tf.matmul(state, w1) + b1)
		with tf.variable_scope('l3'):
			w3 = tf.get_variable('w3', [units_1, a_d], initializer=w_i, collections=c_names, regularizer=reg)
			b3 = tf.get_variable('b3', [1, a_d], initializer=b_i, collections=c_names, regularizer=reg)
			dense3 = tf.matmul(dense1, w3) + b3
		return dense3
	...
	def Q_select(self):
		with tf.variable_scope('select_net') as scope:
			...
			return self.build_layers(self.select_input, c_names, 200, w_i, b_i, reg)
	...
	def Q_eval(self):
		with tf.variable_scope('eval_net') as scope:
			...
			w_i = tf.random_uniform_initializer(-0.1, 0.1)
			b_i = tf.constant_initializer(0.1)
			act_fn_h = tf.nn.sigmoid
			return self.build_layers(self.eval_input, c_names, 200, w_i, b_i, act_fn_h)
	...
	def egreedy_action(self, state):
		q_values = self.Q_select.eval(feed_dict={self.select_input: [state]})[0]
		action = np.argmax(q_values)
		if random.random() <= self.epsilon:
			return random.randint(0, self.action_dim - 1), np.max(q_values)
