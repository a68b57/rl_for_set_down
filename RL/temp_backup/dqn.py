...
class AbstractDQNAgent(Agent):
	"""Write me
	"""
	def __init__(self, nb_actions, memory, memory_expert=None, gamma=.99, batch_size=32, nb_steps_warmup=1000,
		...
		# Related objects.
		self.memory = memory
		self.memory_expert = memory_expert
	...
	def get_config(self):
		return {
			...
			'memory_expert':get_object_config(self.memory_expert),
		}
...
class DQNAgent(AbstractDQNAgent):
	...
	def compile(self, optimizer, metrics=[]):
		...
		def jeq_with_error_margin(model_q_values, mask):
			Q_s_ae = K.sum(model_q_values*mask, axis=-1)
			margin = 0.4
			return K.max(model_q_values + margin - mask * margin, axis=-1) - Q_s_ae

		def clipped_masked_error(args):
			# TODO: implement new error here, this will return the mean of the batch eventually

			Q_s_a, target, mask = args

			loss = huber_loss(target, Q_s_a, self.delta_clip)

			loss *= mask  # apply element-wise mask
			JDQ = K.sum(loss, axis=-1)


			JEQ = jeq_with_error_margin(Q_s_a, mask)
		...
		Q_s_a = self.model.output
		targets = Input(name='targets', shape=(self.nb_actions,))
		mask = Input(name='mask', shape=(self.nb_actions,))
		loss_out = Lambda(clipped_masked_error, output_shape=(1,), name='loss_out')([Q_s_a, targets, mask])
		ins = [self.model.input] if type(self.model.input) is not list else self.model.input
		# trainable_model = Model(inputs=ins + [targets, mask], outputs=[loss_out, Q_s_a])
		trainable_model = Model(inputs=ins + [targets, mask], outputs=[loss_out, Q_s_a])

		assert len(trainable_model.output_names) == 2
		combined_metrics = {trainable_model.output_names[1]: metrics}
		losses = [
			# here the "y_true" and "y_pred" has nothing to do with the above y_pred and y_true, they are just two
			# variables in lambda function, they are fed by trainable_model.train_on_batch function
			# in this case, y_pred is the output of trainable_model which is loss_out and Q_s_a(model output)
			lambda y_true, y_pred: y_pred,  # loss is computed in Lambda layer, for the first output
			lambda y_true, y_pred: K.zeros_like(y_pred),  # we only include this for the metrics, for the second output
		]
	...
	def forward(self, observation, env):
		...
		if self.training:
			...
			# TODO: use MCTS
			# if env.get_holding_step(action) == 15:
			#     mc = mcts.MCTS(simulator=env, rollout_policy_model=self.model, max_nb_sims=10,
			#                    current_state=self.process_state_batch([state]),
			#                 max_depth=2)
			#     action = mc.uct()
	...
	def get_batch(self, experiences):
		...
		return state0_batch, state1_batch, action_batch, terminal1_batch, reward_batch

	def compute_loss_inputs(self, state0_batch, state1_batch, action_batch, terminal1_batch, reward_batch):
		...
		return targets, masks, dummy_targets, self.model.predict_on_batch(state0_batch)
	...
	def backward(self, reward, terminal):
		...
		if self.step % self.memory_interval == 0:
			...
			# TODO: just for collecting expert's transition
		...
		if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
			...
			# pre-train the network only use expert experience for 50,000 updates
			if self.step <= 50000:
				sample_ratio = 0.97
			else:
				sample_ratio = 0.3

			expert_experience_size = int(self.batch_size*sample_ratio)

			experiences_self = self.memory.sample(self.batch_size-expert_experience_size)
			assert len(experiences_self) == self.batch_size - expert_experience_size

			experiences_expert = self.memory_expert.sample(expert_experience_size)


			experiences = experiences_self + experiences_expert

			state0_batch, state1_batch, action_batch, terminal1_batch, reward_batch = self.get_batch(experiences)

			targets, masks, dummy_targets, q_values_state0 = self.compute_loss_inputs(state0_batch, state1_batch,
			                                                                    action_batch,
			                                                         terminal1_batch, reward_batch)



			# TODO: implement the new error for expert experiences
			# TODO: implement new error as backend operations in compile not here
			...
			# ins + [targets,masks] is the "y_pred" (x) part in the loss function of compile,
			# [dummy_targets,targets] is "y_true" (y) part in loss function of compile,
			# here "y_true" is not necessary because real "y_true" is computed by real "y_pred" in Lambda layer of
			# trainable_model
