...
class TestLogger(Callback):
    ...
    def on_episode_end(self, episode, logs):
        template = 'episode {0}: reward: {1:.3f}, steps: {2}, vel: {3:.3f}'
        variables = [
            episode + 1,
            logs['episode_reward'],
            logs['nb_steps'],
        ...
        # if logs['episode_reward']==-99:
	     #    self.env.plot(show_motion=True)
...
class TrainEpisodeLogger(Callback):
    ...
    def on_episode_end(self, episode, logs):
        ...
        template = '{step: ' + nb_step_digits + 'd}/{nb_steps}: episode: {episode}, episode steps: {episode_steps}, episode reward: {episode_reward:.3f}, {metrics}, mean reward: {reward_mean:.3f} [{reward_min:.3f}, {reward_max:.3f}]'
        variables = {
            'step': self.step,
        ...
        print(template.format(**variables))
        # if np.sum(self.rewards[episode]) > 100:
        	# self.env.plot(show_motion=True)
...
class MemoryIntervalCheckpoint(Callback):
	...
	def on_step_end(self, step, logs={}):
		...
		with open(filepath, 'wb') as output:
			pickle.dump(self.model.memory, output, pickle.HIGHEST_PROTOCOL)
